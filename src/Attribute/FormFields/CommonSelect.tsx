...
export const CommonValidationSelect: FC<CommonValidationSelectProps> = ({ name, label, options = [], multiple = false, placeholder, disabled, isLoading }) => {
  ...
  return (
    <Form.Item label={label} validateStatus={meta.touched && meta.error ? "error" : ""} help={meta.touched && meta.error} >
      <Select mode={multiple ? "multiple" : undefined} value={value} options={options.map((o: SelectOptionType) => ({ label: o.label, value: o.value, }))} placeholder={placeholder} disabled={disabled} loading={isLoading} onChange={(val) => { helpers.setValue(val); setFieldValue(name, val); }} onBlur={() => helpers.setTouched(true)} allowClear />
    </Form.Item>
  );
};
