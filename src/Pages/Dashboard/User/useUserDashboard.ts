...
export const useUserDashboard = () => {
    ...
    const { data: transResponse, isLoading: transLoading, refetch: refetchTrans, } = Queries.useGetTransaction({
        limit: 50,
    });
    ...
    const userTransactions = useMemo(() => {
        return rawTransactions.filter((t: any) => {
            const transactionUserId = t.userId || t.user?._id || t.user?.id;
    ...
    const { cashflowData, totalDeposit, totalWithdraw } = useMemo(() => {
        const map = new Map<
            string,
            {
                deposit: number;
        ...
        normalized.forEach((t) => {
            ...
            if (!map.has(date)) {
                map.set(date, { deposit: 0, withdraw: 0 });
            }
        ...
        let wit = 0;
        map.forEach((e, d) => {
            flat.push({ date: d, value: e.deposit, type: "deposit" });
            flat.push({ date: d, value: e.withdraw, type: "withdraw" });
            dep += e.deposit;
            wit += e.withdraw;
        });
        return { cashflowData: flat, totalDeposit: dep, totalWithdraw: wit };
    }, [normalized]);
    ...
    return {
        ...
        loading: transLoading || walletLoading,
        refetch: () => {
            refetchTrans?.();
            refetchWallet?.();
        },
